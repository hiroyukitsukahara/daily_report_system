...
@NamedQueries({
    ...
    @NamedQuery(
            name = JpaConst.Q_EMP_COUNT_REGISTERED_BY_CODE,         //指定された社員番号がすでにデータベースに存在しているかを調べている
...
public class Employee {
     ...
     * 社員番号
     */
    @Column(name = JpaConst.EMP_COL_CODE, nullable = false, unique = true) //unique = trueは一意制約（すでに存在している社員番号は登録できない旨をデータベースに指定するための設定）
    private String code;
